   ...
   case class Stream[A : Manifest](stream: StagedStream[Rep[A]]) {
      ...
      def makeLinear[A : Manifest](stream: StagedStream[Rep[A]]): Producer[Rep[A]] = {
        stream match {
          ...
          case Nested(producer, nestedf) => {
            def makeAdvanced[A](advf: Var[Int => Unit], k: (A => Rep[Unit]), stream: StagedStream[A]): Rep[Unit] = {
              stream match {
                case Linear(producer) => {
                  producer.card match {
                    case AtMost1 =>
                      producer.init(st => {
                        if(producer.hasNext(st))
                          producer.step(st, k)
                            ...
                            val f: Rep[Int => Unit] = advf
                            f(42)
                        }
                      })
                    case Many =>
                      producer.init(st => {
                        val oldAdvf: Rep[Int => Unit] = advf
                        def f: Rep[Int => Unit] = fun { (_ : Rep[Int]) =>
                          if(producer.hasNext(st))
                            producer.step(st, k)
                          else {
                            advf = oldAdvf
            ...
            new Producer[Rep[A]] {
              type St = (Var[Boolean], Var[A], Var[Int => Unit])

              val card = Many
              def init(k: St => Rep[Unit]): Rep[Unit] = {
                producer.init(st => {
                  var advf: Var[Int => Unit] = fun { (_ : Rep[Int]) => () }
